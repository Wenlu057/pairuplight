...
Modified based on MA2C's codebase.
@author: Wenlu Du
"""
...
def parse_args():
    default_base_dir = "Result/"
    default_config_dir = "config/config_ia2c_large.ini"
...
def train(args):
    ...
    if env.agent == 'a2c':
        ...
        model = IA2C(env.n_s, env.n_a, env.n_w, env.n_n, len(env.node_names), total_step,
                     config['MODEL_CONFIG'], seed=seed, checkpoint_dir=dirs['model'])
    elif env.agent == 'ma2c':
    ...
    trainer = Trainer(env, model, global_counter, output_path=dirs['data'])
    trainer.run()
    ...
    # for policy in model.policy_ls:
    #     torch.save(policy.state_dict(),dirs['model'])
...
def evaluate_fn(agent_dir, output_dir, seeds, port, demo, policy_type):
    ...
    if agent != 'greedy':
        ...
        if agent == 'a2c':
            ...
            model = IA2C(env.n_s_ls, env.n_a_ls, env.n_w_ls, env.n_n_ls, config['MODEL_CONFIG'])
        elif agent == 'ma2c':
