...
class OnPolicyBuffer(TransBuffer):
    def __init__(self, gamma, lambd, sample_size):
        self.gamma = gamma
        self.lambd = lambd
        self.reset()
        self.T = sample_size
    ...
    def add_transition(self, ob, a, r, v, mask, hidden, messages):
        self.obs.append(ob)
        self.acts.append(a)
        self.rs.append(r)
        self.vs.append(v)
    ...
    def gae(self):
        T = len(self.rs)
        rs = ((self.rs - np.array(-86)) / np.array(86)) * 2 - 1
        rs = torch.tensor(np.array(rs))
        vs = torch.stack(self.vs)

        self.advantages = torch.zeros(T, 1)
        advantage_t = torch.zeros(1)
        for t in reversed(range(T)):
            delta = rs[t] + vs[t + 1].data * self.gamma * self.masks[t] - vs[t].data
            advantage_t = delta + advantage_t * self.gamma * self.lambd * self.masks[t]
            self.advantages[t] = advantage_t
        self.returns = vs[:T].data + self.advantages
    ...
    def sample_transition(self, R, discrete=True):
        ...
        if discrete:
            acts = torch.tensor(np.array(self.acts))
        else:
            acts = np.array(self.acts, dtype=np.float32)
        masks = torch.stack(self.masks)
        hidden = torch.stack(self.hiddens)
        message = torch.stack(self.messages)
        self.reset()
        return obs, acts, masks, self.returns, self.advantages, hidden, message
...
class Scheduler:
    ...
    def get(self):
        if self.decay == 'linear':
            return lambda a: self.val * (1. - a)
        else:
            return lambda a: self.val
...
class PPOObjective(nn.Module):
    def forward(self, clip, pi, v, pi_old, v_old, action, advantage, returns):
        ...
        """
        prob = Fnn.softmax(pi, dim=1)
        log_prob = Fnn.log_softmax(pi,dim=1)
        action_prob = prob.gather(1, action)

        prob_old = Fnn.softmax(pi_old,dim=1)
